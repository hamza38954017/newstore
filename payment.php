...
if ($fromCart) {
    $cartKeys = fbGet("cart/$sid") ?? [];
    foreach (array_keys($cartKeys) as $pk) {
        $p = fbGet("photos/$pk");
        if ($p && ($p['status']??'') === 'active') {
            $p['id'] = $pk; $cartPhotos[] = $p;
            $totalAmount += (float)($p['price'] ?? 0);
        }
    }
    if (empty($cartPhotos)) { header('Location: index.php'); exit; }
} elseif ($photoId) {
    $photo = fbGet("photos/$photoId");
    if (!$photo || ($photo['status']??'') !== 'active') { header('Location: index.php'); exit; }
    $photo['id'] = $photoId;
    $totalAmount = (float)($photo['price'] ?? 0);
...
// ── Order ID (once per session / reset after timeout) ─────────────────────
...
$timeRemaining = max(0, $timeout - (time() - ($_SESSION['payment_time'] ?? time())));

// ── UTR Submission ────────────────────────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['utr'])) {
    header('Content-Type: application/json');
    $utr = preg_replace('/\D/', '', $_POST['utr'] ?? '');
    ...
    fbUpdate("payment_sessions/$orderId", ['utr_submitted' => true]);
    if ($fromCart) fbDelete("cart/$sid");
...
if (isset($_POST['action']) && $_POST['action'] === 'timeout_notify') {
    header('Content-Type: application/json');
    fbUpdate("payment_sessions/$orderId", ['timed_out' => true]);
    echo json_encode(['ok' => true]); exit;
}

// ── UPI deep links ────────────────────────────────────────────────────────
$amt        = number_format($totalAmount, 2, '.', '');
$upiBase    = 'pa='.urlencode($upiId).'&pn='.urlencode($upiName).'&mc=0000&mode=02&purpose=00&am='.$amt.'&cu=INR';
