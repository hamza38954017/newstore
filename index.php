...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] === 'add_cart' && isset($_GET['photo_id'])) {
        $pid = $_GET['photo_id'];
        fbSet("cart/$sid/$pid", 1);
        fbPush('photo_analytics', ['photo_id'=>$pid,'event_type'=>'add_to_cart','session_id'=>$sid,'ip_address'=>$ip,'created_at'=>date('Y-m-d H:i:s')]);
        $cnt = count(fbGet("cart/$sid") ?? []);
    ...
    if ($_GET['ajax'] === 'cart_items') {
        $keys  = fbGet("cart/$sid") ?? [];
        $items = [];
        foreach (array_keys($keys) as $pk) {
            $p = fbGet("photos/$pk");
            if ($p && ($p['status']??'') === 'active') { $p['id'] = $pk; $items[] = $p; }
        }
    ...
    if ($_GET['ajax'] === 'mark_notif_read' && isset($_GET['id'])) {
        fbUpdate('notifications/'.$_GET['id'], ['is_read'=>true]);
        echo json_encode(['success'=>true]);
        exit;
    }

    if ($_GET['ajax'] === 'track' && isset($_GET['event'],$_GET['photo_id'])) {
        $ev = in_array($_GET['event'],['view','purchase_click','add_to_cart']) ? $_GET['event'] : 'view';
        fbPush('photo_analytics', ['photo_id'=>$_GET['photo_id'],'event_type'=>$ev,'session_id'=>$sid,'ip_address'=>$ip,'created_at'=>date('Y-m-d H:i:s')]);
        echo json_encode(['ok'=>true]);
        exit;
    }
    exit;
...
if ($slug) {
    $allPhotos = fbGet('photos') ?? [];
    foreach ($allPhotos as $k => $p) {
    ...
    if ($photo) {
        $page = 'photo';
        fbPush('photo_analytics', ['photo_id'=>$photo['id'],'photo_slug'=>$photo['slug'],'event_type'=>'view','session_id'=>session_id(),'ip_address'=>getClientIP(),'created_at'=>date('Y-m-d H:i:s')]);
        foreach ($allPhotos as $k => $p) {
            if (!is_array($p) || $k===$photo['id'] || ($p['status']??'')==='') continue;
            if (($p['type']??'') === ($photo['type']??'') || ($p['country']??'') === ($photo['country']??'')) {
                $p['id'] = $k;
                $related[] = $p;
                if (count($related) >= 6) break;
            }
        }
        $allRevs = fbGet('reviews') ?? [];
        foreach ($allRevs as $k => $r) {
            if (is_array($r) && ($r['photo_id']??'') === $photo['id'] && ($r['status']??'') === 'approved') {
                $r['id'] = $k;
                $reviews[] = $r;
            }
        }
...
if ($page === 'home') {
    if (empty($allPhotos)) $allPhotos = fbGet('photos') ?? [];
    $typeSet = []; $countrySet = [];
    foreach ($allPhotos as $k => $p) {
        if (!is_array($p) || ($p['status']??'') !== 'active') continue;
        if ($p['type']??'')    $typeSet[$p['type']] = 1;
        if ($p['country']??'') $countrySet[$p['country']] = 1;
        if ($search        && stripos(($p['title']??'').($p['tags']??'').($p['country']??''), $search) === false) continue;
        if ($filter_type   && ($p['type']??'')    !== $filter_type)    continue;
        if ($filter_country&& ($p['country']??'') !== $filter_country) continue;
        $p['id'] = $k;
        $photos[] = $p;
    }
    $types     = array_keys($typeSet);     sort($types);
    $countries = array_keys($countrySet);  sort($countries);
    usort($photos, fn($a,$b) => ($b['featured']??0)<=>($a['featured']??0) ?: strcmp($b['created_at']??'',$a['created_at']??''));

    $allSlides = fbGet('carousel_slides') ?? [];
    foreach ($allSlides as $k => $sl) {
        if (is_array($sl) && ($sl['status']??'') === 'active') { $sl['id']=$k; $slides[]=$sl; }
    }
    usort($slides, fn($a,$b) => ($a['sort_order']??0)<=>($b['sort_order']??0));
}

// ── Counts ────────────────────────────────────────────────────────────────
$cartData   = fbGet('cart/'.session_id()) ?? [];
$cartCnt    = count($cartData);
$allNotifs  = fbGet('notifications') ?? [];
$notifCount = count(array_filter($allNotifs, fn($n) => is_array($n) && empty($n['is_read'])));
