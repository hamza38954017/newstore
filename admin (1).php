...
if (isAdminLoggedIn() && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'save_settings') {
        $fields = ['site_name','site_title','copyright_name','copyright_year',
                   'upi_id','upi_name','telegram_url','telegram_support_user',
                   'instagram_url','youtube_url','facebook_url','twitter_url',
                   'primary_color','secondary_color','accent_color','bg_color','text_color',
                   'payment_timeout','meta_description','privacy_policy','terms_conditions',
                   'guide_image_url','faq_1_q','faq_1_a','faq_2_q','faq_2_a',
                   'faq_3_q','faq_3_a','faq_4_q','faq_4_a','faq_5_q','faq_5_a',
                   'faq_6_q','faq_6_a','admin_username','admin_password'];
        $save = [];
        foreach ($fields as $f) {
            if (!isset($_POST[$f])) continue;
            if ($f === 'admin_password' && $_POST[$f] === '') continue;
            $save[$f] = $_POST[$f];
        }
        fbUpdate('settings', $save);
        echo json_encode(['success'=>true,'msg'=>'Settings saved!']); exit;
    }

    if ($act === 'save_photo') {
        $fbKey = $_POST['photo_id'] ?? '';   // empty = new photo
        $title = trim($_POST['title'] ?? '');
        $slug  = trim($_POST['slug']  ?? '') ?: generateSlug($title);
        // Ensure unique slug
        $all = fbGet('photos') ?? [];
        foreach ($all as $k => $p) {
            if (is_array($p) && ($p['slug']??'') === $slug && $k !== $fbKey) { $slug .= '-'.time(); break; }
        }
        $data = [
            'title'            => $title,
            'slug'             => $slug,
        ...
        if ($fbKey) {
            fbUpdate("photos/$fbKey", $data);
            echo json_encode(['success'=>true,'id'=>$fbKey]); exit;
        } else {
            $data['created_at'] = date('Y-m-d H:i:s');
            $newKey = fbPush('photos', $data);
            if ($newKey) fbUpdate("photos/$newKey", ['id'=>$newKey]);
            echo json_encode(['success'=>true,'id'=>$newKey]); exit;
        }
    }

    if ($act === 'delete_photo') {
        fbDelete('photos/'.($_POST['photo_id']??''));
        echo json_encode(['success'=>true]); exit;
    }

    if ($act === 'update_purchase') {
        $oid = $_POST['purchase_id'] ?? '';   // order_id is the firebase key
        fbUpdate("purchases/$oid", ['payment_status'=>$_POST['status']??'pending','notes'=>$_POST['notes']??'','updated_at'=>date('Y-m-d H:i:s')]);
        echo json_encode(['success'=>true]); exit;
    }

    if ($act === 'add_notification') {
        fbPush('notifications', ['title'=>$_POST['title']??'','message'=>$_POST['message']??'','type'=>$_POST['type']??'info','is_read'=>false,'created_at'=>date('Y-m-d H:i:s')]);
        echo json_encode(['success'=>true]); exit;
    }

    if ($act === 'delete_notification') {
        fbDelete('notifications/'.($_POST['notif_id']??''));
        echo json_encode(['success'=>true]); exit;
    }

    if ($act === 'save_review') {
        $rfk  = $_POST['review_id'] ?? '';
        $data = ['photo_id'=>$_POST['photo_id']??'','reviewer_name'=>$_POST['reviewer_name']??'',
                 'rating'=>(int)($_POST['rating']??5),'comment'=>$_POST['comment']??'',
                 'status'=>$_POST['status']??'approved','updated_at'=>date('Y-m-d H:i:s')];
        if ($rfk) { fbUpdate("reviews/$rfk", $data); }
        else { $data['created_at']=date('Y-m-d H:i:s'); fbPush('reviews',$data); }
        echo json_encode(['success'=>true]); exit;
    }

    if ($act === 'delete_review') {
        fbDelete('reviews/'.($_POST['review_id']??''));
        echo json_encode(['success'=>true]); exit;
    }

    if ($act === 'save_slide') {
        $sfk  = $_POST['slide_id'] ?? '';
        $data = ['image_url'=>$_POST['image_url']??'','caption'=>$_POST['caption']??'',
                 'sort_order'=>(int)($_POST['sort_order']??0),'status'=>$_POST['status']??'active'];
        if ($sfk) { fbUpdate("carousel_slides/$sfk", $data); }
        else {
            if (count(fbGet('carousel_slides') ?? []) >= 15) { echo json_encode(['success'=>false,'msg'=>'Max 15 slides.']); exit; }
            fbPush('carousel_slides', $data);
        }
        echo json_encode(['success'=>true]); exit;
    }

    if ($act === 'delete_slide') {
        fbDelete('carousel_slides/'.($_POST['slide_id']??''));
        echo json_encode(['success'=>true]); exit;
    }
...
$vPage=max(1,(int)($_GET['vpage']??1)); $vTotal=$vPages=0;

if (isAdminLoggedIn()) {
    $s = getAllSettings();

    // Photos
    foreach (fbGet('photos') ?? [] as $k => $p) {
    ...
    $pendingPurch    = count(array_filter($purchases, fn($p) => ($p['payment_status']??'')==='pending'));
    $verifiedRevenue = array_sum(array_map(fn($p) => ($p['payment_status']??'')==='verified' ? (float)($p['amount']??0) : 0, $purchases));
    $purchasesUTR    = array_values(array_filter($purchases, fn($p) => !empty($p['utr_number'])));
    $recentRevUsers  = array_slice(array_values(array_filter($purchases, fn($p) => ($p['payment_status']??'')==='verified')), 0, 20);
    $utrSubmitted    = count($purchasesUTR);

    // Payment sessions without UTR
    foreach (fbGet('payment_sessions') ?? [] as $k => $ps) {
        if (is_array($ps) && empty($ps['utr_submitted'])) { $ps['id']=$k; $sessionsNoUTR[]=$ps; }
    }
    $utrNotSubmitted = count($sessionsNoUTR);

    // Reviews
    foreach (fbGet('reviews') ?? [] as $k => $r) {
        if (is_array($r)) { $r['id']=$k; $reviews[]=$r; }
    }

    // Notifications
    foreach (fbGet('notifications') ?? [] as $k => $n) {
        if (is_array($n)) { $n['id']=$k; $notifications[]=$n; }
    }
    usort($notifications, fn($a,$b) => strcmp($b['created_at']??'',$a['created_at']??''));

    // Slides
    foreach (fbGet('carousel_slides') ?? [] as $k => $sl) {
        if (is_array($sl)) { $sl['id']=$k; $slides[]=$sl; }
    }
    usort($slides, fn($a,$b) => ($a['sort_order']??0)<=>($b['sort_order']??0));

    // Analytics — build per-photo stats
    $pStats = [];
    foreach (fbGet('photo_analytics') ?? [] as $a) {
        if (!is_array($a)) continue;
        $pid = $a['photo_id']??''; $ev = $a['event_type']??'';
        if (!isset($pStats[$pid])) $pStats[$pid] = ['views'=>0,'purchase_clicks'=>0,'cart_adds'=>0];
        if ($ev==='view')            $pStats[$pid]['views']++;
        elseif ($ev==='purchase_click') $pStats[$pid]['purchase_clicks']++;
        elseif ($ev==='add_to_cart') $pStats[$pid]['cart_adds']++;
    }
    foreach ($photos as $p) {
        $pk = $p['id']??'';
        $photoStats[] = ['id'=>$pk,'title'=>$p['title']??'','slug'=>$p['slug']??'',
                         'views'=>$pStats[$pk]['views']??0,
                         'purchase_clicks'=>$pStats[$pk]['purchase_clicks']??0,
                         'cart_adds'=>$pStats[$pk]['cart_adds']??0];
    }
    usort($photoStats, fn($a,$b) => $b['views']<=>$a['views']);

    // Visitor logs — unique IPs + paginated list
    $allVis = array_values(array_filter(fbGet('visitor_logs') ?? [], 'is_array'));
    $totalVisitors = count(array_unique(array_column($allVis, 'ip_address')));
    $totalViews    = array_sum(array_column($photoStats, 'views'));
    usort($allVis, fn($a,$b) => strcmp($b['visited_at']??'',$a['visited_at']??''));
    $vTotal   = count($allVis);
    $vLimit   = 50;
    $vPages   = max(1, (int)ceil($vTotal / $vLimit));
    $visitors = array_slice($allVis, ($vPage-1)*$vLimit, $vLimit);
}
?>
<!DOCTYPE html>
<html lang="en">
